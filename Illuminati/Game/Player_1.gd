...
onready var Player1illuminati : TextureButton = $Player1illuminati
...
func _ready():
	...
	print(testarray)
	player1 = testarray[1]
	player1.getIlluminati().updateMoney()
	
	# This will iterate through group cards and add income
	if (testarray[1].groupCards.size() > 0):
		for i in testarray[1].groupCards:
			i.money += i.income
			
	print("Bank: " + str(player1.IlluminatiCard.money))
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	var path = "res://Card Assets Folder/" + players[1].IlluminatiCard.getname() + ".JPG" # path of their Illuminati Card
	var image = load(path)
	Player1illuminati.set_normal_texture(image)#setting the button's image
	#setting the button size
	Player1illuminati.set_expand(true) 
	Player1illuminati.set_stretch_mode(0)
...
func _on_Player1illuminati_pressed():
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	global.attacker = players[1].IlluminatiCard#set attacker to the card you chose
