...
onready var Player6illuminati : TextureButton = $Player6illuminati
...
func _ready():
	...
	print(testarray)
	player6 = testarray[6]
	player6.getIlluminati().updateMoney()
	
	# This will iterate through group cards and add income
	if (testarray[6].groupCards.size() > 0):
		for i in testarray[6].groupCards:
			i.money += i.income
			
	print("Bank: " + str(player6.IlluminatiCard.money))
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	var path = "res://Card Assets Folder/" + players[6].IlluminatiCard.getname() + ".JPG" # path of their Illuminati Card
	var image = load(path)
	Player6illuminati.set_normal_texture(image)#setting the button's image
	#setting the button size
	Player6illuminati.set_expand(true) 
	Player6illuminati.set_stretch_mode(0)
...
func _on_Player6illuminati_pressed():
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	global.attacker = players[6].IlluminatiCard#set attacker to the card you chose
