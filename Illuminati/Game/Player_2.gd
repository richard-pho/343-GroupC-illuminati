...
onready var Player2illuminati : TextureButton = $Player2illuminati
...
func _ready():
	...
	print(testarray)
	player2 = testarray[2]
	player2.getIlluminati().updateMoney()
	
	# This will iterate through group cards and add income
	if (testarray[2].groupCards.size() > 0):
		for i in testarray[2].groupCards:
			i.money += i.income
			
	print("Bank: " + str(player2.IlluminatiCard.money))
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	var path = "res://Card Assets Folder/" + players[2].IlluminatiCard.getname() + ".JPG" # path of their Illuminati Card
	var image = load(path)
	Player2illuminati.set_normal_texture(image)#setting the button's image
	#setting the button size
	Player2illuminati.set_expand(true) 
	Player2illuminati.set_stretch_mode(0)
...
func _on_Player2illuminati_pressed():
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	global.attacker = players[2].IlluminatiCard#set attacker to the card you chose
