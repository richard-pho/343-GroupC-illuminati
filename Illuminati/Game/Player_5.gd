...
onready var Player5illuminati : TextureButton = $Player5illuminati
...
func _ready():
	...
	print(testarray)
	player5 = testarray[5]
	player5.getIlluminati().updateMoney()
	
	# This will iterate through group cards and add income
	if (testarray[5].groupCards.size() > 0):
		for i in testarray[5].groupCards:
			i.money += i.income
			
	print("Bank: " +  str(player5.IlluminatiCard.money))
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	var path = "res://Card Assets Folder/" + players[5].IlluminatiCard.getname() + ".JPG" # path of their Illuminati Card
	var image = load(path)
	Player5illuminati.set_normal_texture(image)#setting the button's image
	#setting the button size
	Player5illuminati.set_expand(true) 
	Player5illuminati.set_stretch_mode(0)
...
func _on_Player5illuminati_pressed():
	var global = get_node("/root/globals")
	var players = global.players # gets the array of players from global
	global.attacker = players[5].IlluminatiCard#set attacker to the card you chose
